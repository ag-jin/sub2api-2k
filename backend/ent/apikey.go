...
type APIKey struct {
	...
	// Pricing plan this API key belongs to (nil = legacy group billing)
...
type APIKeyEdges struct {
	...
	// PricingPlan holds the value of the pricing_plan edge.
...
// PricingPlanOrErr returns the PricingPlan value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e APIKeyEdges) PricingPlanOrErr() (*PricingPlan, error) {
	if e.PricingPlan != nil {
		return e.PricingPlan, nil
	} else if e.loadedTypes[2] {
		return nil, &NotFoundError{label: pricingplan.Label}
	}
	return nil, &NotLoadedError{edge: "pricing_plan"}
}
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldID, apikey.FieldUserID, apikey.FieldGroupID, apikey.FieldPricingPlanID:
...
func (_m *APIKey) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case apikey.FieldPricingPlanID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field pricing_plan_id", values[i])
			} else if value.Valid {
				_m.PricingPlanID = new(int64)
				*_m.PricingPlanID = value.Int64
			}
...
// QueryPricingPlan queries the "pricing_plan" edge of the APIKey entity.
func (_m *APIKey) QueryPricingPlan() *PricingPlanQuery {
	return NewAPIKeyClient(_m.config).QueryPricingPlan(_m)
}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	if v := _m.PricingPlanID; v != nil {
		builder.WriteString("pricing_plan_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
