...
func (APIKey) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("group_id").
			Optional().
			Nillable(),
		field.Int64("pricing_plan_id").
			Optional().
			Nillable().
			Comment("Pricing plan this API key belongs to (nil = legacy group billing)"),
		field.String("status").
			MaxLen(20).
...
func (APIKey) Edges() []ent.Edge {
	return []ent.Edge{
		...
		edge.From("group", Group.Type).
			...
			Field("group_id").
			Unique(),
		edge.From("pricing_plan", PricingPlan.Type).
			Ref("api_keys").
