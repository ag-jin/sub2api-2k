...
// 分组/渠道/套餐显式定价优先；否则在规则存在且分组长上下文开关开启时生效。
func legacyLongContextApplies(resolved *ResolvedPricing, group *Group, rule *LegacyLongContextRule) bool {
	...
	if resolved != nil && (resolved.Source == PricingSourceGroup || resolved.Source == PricingSourceChannel || resolved.Source == PricingSourcePlan) {
		return false
	}
...
//  1. 分组/渠道/套餐显式定价 → 统一计费（区间、分组卡、目录阶梯均在其中）；
