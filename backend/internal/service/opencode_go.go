...
// IsOpenCodeGo 报告账号是否为 OpenCode 平台。
//
// 兼容两种落库取值：上游主干的 "opencode_go"（PlatformOpenCodeGo）与 card 血统
// 生产存量的 "opencode"（PlatformOpenCode，来自本仓早前落地的同名平台）。两者是
// 同一上游服务的两种写法，账号类型（Zen/Go）均从 credentials.account_mode 读取、
// 未设置时按 Go 处理，因此共用同一套处理链（端点分派 / 会话头 / UA / 用量）。
// 待存量账号迁移到 opencode_go 后，可去掉对 PlatformOpenCode 的兼容。
func (a *Account) IsOpenCodeGo() bool {
	return a != nil && (a.Platform == PlatformOpenCodeGo || a.Platform == PlatformOpenCode)
}
