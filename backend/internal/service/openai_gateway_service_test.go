...
func newErrAfterReader(body string, err error) io.ReadCloser {
	return &errAfterReader{r: strings.NewReader(body), err: err}
}
...
func TestBuildOpenAITextContinuationRequestAddsInstructions(t *testing.T) {
	body := []byte(`{"model":"gpt-5.4","stream":true,"instructions":"answer in Chinese","input":"continue this"}`)

	got, err := BuildOpenAITextContinuationRequest(body, "already emitted tail")

	require.NoError(t, err)
	require.True(t, gjson.GetBytes(got, "stream").Bool())
	instructions := gjson.GetBytes(got, "instructions").String()
	require.Contains(t, instructions, "answer in Chinese")
	require.Contains(t, instructions, "already-sent text tail")
	require.Contains(t, instructions, "already emitted tail")
	require.Contains(t, instructions, "Do not repeat text already emitted")
	require.NotEqual(t, string(body), string(got), "continuation must not be a bare replay")
}

func TestBuildOpenAITextContinuationRequestAddsChatSystemMessage(t *testing.T) {
	body := []byte(`{"model":"gpt-5.4","messages":[{"role":"user","content":"hello"}],"stream":true}`)

	got, err := BuildOpenAITextContinuationRequest(body, "already emitted chat tail")
	...
	content := gjson.GetBytes(got, "messages.1.content").String()
	require.Contains(t, content, "already-sent text tail")
	require.Contains(t, content, "already emitted chat tail")
	require.Contains(t, content, "Do not repeat text already emitted")
}
...
func TestOpenAIStreamingReadErrorAfterOutputReturnsContinuationFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
	...
	body := "data: {\"type\":\"response.output_text.delta\",\"delta\":\"hello continuation tail\"}\n\n"
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Body:       newErrAfterReader(body, io.ErrUnexpectedEOF),
		Header:     http.Header{"X-Request-Id": []string{"rid-post-token-disconnect"}},
	}
	...
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	require.True(t, failoverErr.PostFirstTokenContinuation)
	require.Equal(t, "hello continuation tail", failoverErr.ContinuationTail)
	require.Contains(t, rec.Body.String(), "hello continuation tail")
