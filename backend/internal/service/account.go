...
// IsOpenCode 只匹配 card 血统的存量落库值 "opencode"（PlatformOpenCode）。
//
// 上游主干用 "opencode_go"（PlatformOpenCodeGo）并有完整实现：IsOpenCodeGo()
// 同时接受两种取值，因此 opencode_go 与存量 opencode 账号在上游代码路径里都按
// OpenCode 处理。本谓词保留"只认存量值"的语义，专供 card 血统的历史分支使用——
// 若让它也匹配 opencode_go，card 的旧分支会抢在上游协议分流之前执行，把
// opencode_go 的按模型原生协议路由退化成单一 Chat Completions（2026-09-17
// 合并验证时实测，见 account_test_service.go 的测试入口分派顺序）。
...
func (a *Account) IsOpenAICompatible() bool {
	return a != nil && (a.Platform == PlatformOpenAI || a.Platform == PlatformGrok ||
		a.IsCNProvider() || a.IsOpenCodeAPIKey() || a.IsOpenCodeGo() || a.IsCodeBuddy())
}
...
func (a *Account) GetOpenAIBaseURL() string {
	if !a.IsOpenAI() && !a.IsCNProvider() && !a.IsOpenCodeGo() && !a.IsCodeBuddy() {
		return ""
	}
	...
	case PlatformOpenCode, PlatformOpenCodeGo:
