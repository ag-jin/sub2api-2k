...
func TestOpenAIGatewayService_SelectAccountWithScheduler_CostFactorRanksAfterHealthFilters(t *testing.T) {
	ctx := context.Background()
	groupID := int64(111)
	accounts := []Account{
		{
	...
	require.Equal(t, 2, decision.CandidateCount, "quota-paused cheap account must be hard-filtered before cost ranking")
	require.Equal(t, 1, decision.TopK)
	require.InDelta(t, 0.25, decision.SelectedCostFactor, 1e-12)
	require.InDelta(t, 0.25, decision.CheapestCandidateCostFactor, 1e-12)
	require.True(t, decision.CheapestCandidateHit)
	snapshot := svc.SnapshotOpenAIAccountSchedulerMetrics()
	require.Equal(t, int64(1), snapshot.CheapestHitTotal)
	require.Equal(t, int64(1), snapshot.SelectedCostFactorSampleTotal)
	require.InDelta(t, 0.25, snapshot.SelectedCostFactorAvg, 1e-12)
	if selection.ReleaseFunc != nil {
		selection.ReleaseFunc()
	}
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_CostFactorDoesNotBreakSticky(t *testing.T) {
	ctx := context.Background()
	groupID := int64(112)
	sessionHash := "sticky-cost-preserved"
	accounts := []Account{
		{
			ID:          37201,
			Platform:    PlatformOpenAI,
