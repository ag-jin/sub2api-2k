...
func TestForwardAsChatCompletions_StreamReadErrorAfterOutputReturnsContinuationFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_chat_tail","model":"gpt-5.4","status":"in_progress","output":[]}}`,
		"",
		`data: {"type":"response.output_text.delta","delta":"hello continuation tail"}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_chat_post_token_disconnect"}},
		Body:       newErrAfterReader(upstreamBody, io.ErrUnexpectedEOF),
	}}
