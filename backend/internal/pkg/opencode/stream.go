...
// ConvertStream reads an OpenCode Chat Completions SSE stream from r and writes
// the equivalent Anthropic Messages SSE to w. Chain per chunk:
//
//	Chat Completions chunk (choices[].delta.content / .reasoning_content / .tool_calls)
...
func ConvertStream(r io.Reader, w SSEWriter, model string) (StreamUsage, error) {
	...
	for _, e := range apicompat.FinalizeChatCompletionsResponsesStream(ccState) {
		// DIAG: print finalize event types
		fmt.Fprintf(os.Stderr, "[FINALIZE] type=%s outputIndex=%d args=%q name=%s\n", e.Type, e.OutputIndex, e.Arguments, e.Name)
...
// ConvertNonStream converts a complete OpenCode Chat Completions JSON response
