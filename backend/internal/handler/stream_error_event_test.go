...
func TestPostFirstTokenContinuationBudgetExhaustedEmitsResponseFailed(t *testing.T) {
	c, w := newGinContextForEndpoint(t, EndpointResponses)
	...
	failoverErr := &service.UpstreamFailoverError{
		...
		ContinuationTail:           "already streamed",
	}

	action := fs.HandlePostFirstTokenFailover(c.Request.Context(), service.OpenAIStreamContinuationTextAggressive, 100, failoverErr)
