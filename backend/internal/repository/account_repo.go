...
func (r *accountRepository) ListOAuthRefreshCandidatePage(ctx context.Context, options service.OAuthRefreshPageOptions) (*service.OAuthRefreshCandidatePage, error) {
	...
	if options.IncludeCodeBuddyAuth {
		// CodeBuddy 的 auth JSON 账号以 api_key 类型参与后台 token 轮换。
		clause := " AND (type IN ('oauth', 'setup-token') OR (platform = '" + service.PlatformCodeBuddy + "' AND type = '" + service.AccountTypeAPIKey + "'))"
		query += clause
...
// UpdateCodeBuddyCredentialsIfUnchanged persists provider-issued replacement
// credentials for a CodeBuddy auth-JSON account only while the complete
// credential document and proxy still match the fresh snapshot used by the
// upstream refresh call (mirror of the Grok OAuth success CAS).
func (r *accountRepository) UpdateCodeBuddyCredentialsIfUnchanged(
	ctx context.Context,
	id int64,
	expectedCredentials map[string]any,
	expectedProxyID *int64,
...
// SetCodeBuddyRefreshErrorIfCredentialsUnchanged 是 CodeBuddy 后台刷新的不可重试
// 失败落库（StatusError + 提示重录），仅在凭据文档未被并发修改时生效
// （账号可能正在被重新导入授权，R3-H2 的 CAS 语义）。
func (r *accountRepository) SetCodeBuddyRefreshErrorIfCredentialsUnchanged(
	ctx context.Context,
	id int64,
	expectedCredentials map[string]any,
	expectedProxyID *int64,
...
// SetCodeBuddyRefreshTempUnschedulableIfCredentialsUnchanged applies a bounded
// transient refresh quarantine only while the active CodeBuddy credential
// document still matches the exact upstream attempt (mirror of the Grok
// background-refresh quarantine CAS, 10min temp unschedulable).
func (r *accountRepository) SetCodeBuddyRefreshTempUnschedulableIfCredentialsUnchanged(
	ctx context.Context,
	id int64,
	expectedCredentials map[string]any,
	expectedProxyID *int64,
